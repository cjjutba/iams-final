...
class AnalyticsService:
    ...
    def get_class_overview(
        ...
        q = self.db.query(AttendanceRecord).filter(AttendanceRecord.schedule_id == sid)
        if start_date:
        ...
        enrolled = self.db.query(func.count(Enrollment.id)).filter(Enrollment.schedule_id == sid).scalar() or 0

        early_leaves = (
            self.db.query(func.count(EarlyLeaveEvent.id))
            .join(AttendanceRecord, EarlyLeaveEvent.attendance_id == AttendanceRecord.id)
            .filter(AttendanceRecord.schedule_id == sid)
            .scalar()
            or 0
        )

        anomalies = (
            self.db.query(func.count(AttendanceAnomaly.id)).filter(AttendanceAnomaly.schedule_id == sid).scalar() or 0
        )
    ...
    def get_class_ranking(self, schedule_id: str) -> list[dict[str, Any]]:
        ...
        enrollments = self.db.query(Enrollment).filter(Enrollment.schedule_id == sid).all()
        ...
        for enrollment in enrollments:
            ...
            avg_engagement = None
            if eng_scores:
                avg_engagement = round(sum(e.engagement_score for e in eng_scores) / len(eng_scores), 1)
    ...
    def get_at_risk_students(self, faculty_id: str) -> list[dict[str, Any]]:
        ...
        schedules = self.db.query(Schedule).filter(Schedule.faculty_id == uuid.UUID(faculty_id)).all()
        ...
        for schedule in schedules:
            predictions = (
                ...
                .filter(
                    ...
                    AttendancePrediction.risk_level.in_([RiskLevel.CRITICAL, RiskLevel.HIGH, RiskLevel.MODERATE]),
                )
    ...
    def get_attendance_heatmap(
        self, schedule_id: str, start_date: date | None = None, end_date: date | None = None
    ) -> list[dict[str, Any]]:
    ...
    def get_student_dashboard(self, student_id: str) -> dict[str, Any]:
        ...
        records = self.db.query(AttendanceRecord).filter(AttendanceRecord.student_id == uid).all()
    ...
    def get_student_subject_breakdown(self, student_id: str) -> list[dict[str, Any]]:
        ...
        enrollments = self.db.query(Enrollment).filter(Enrollment.student_id == uid).all()

        breakdown = []
        for enrollment in enrollments:
            schedule = self.db.query(Schedule).filter(Schedule.id == enrollment.schedule_id).first()
            if not schedule:
                continue
