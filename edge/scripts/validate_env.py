...
def check_platform():
    ...
    print("⚠️  Not detected as Raspberry Pi (will proceed anyway)")
    return True
...
def validate_server_url(url: str) -> bool:
    ...
    try:
        ...
        if not all([result.scheme, result.netloc]):
            print("❌ BACKEND_URL: Invalid URL format")
            return False

        if result.scheme not in ["http", "https"]:
            print("❌ BACKEND_URL: Invalid scheme (expected http or https)")
            return False
...
def validate_camera():
    ...
    try:
        ...
        if cap.isOpened():
            ...
            print("   This may be normal if camera is not connected yet")
            return True  # Don't fail validation
    except ImportError:
        print("⚠️  OpenCV not installed (will install from requirements.txt)")
        return True
    except Exception as e:
...
def validate_env():
    ...
    try:
        ...
        print("❌ CAMERA_INDEX: Invalid integer")
        all_valid = False
    ...
    try:
        ...
        print("❌ CAMERA_WIDTH/CAMERA_HEIGHT: Invalid integers")
        all_valid = False
    ...
    try:
        ...
        print("❌ Queue settings: Invalid integers")
        all_valid = False
    ...
    try:
        ...
        print("❌ RETRY_INTERVAL_SECONDS: Invalid integer")
        all_valid = False
    ...
    if room_id:
        ...
        print("⚠️  ROOM_ID: Not set (will detect all faces without room association)")
