...
fun FacultyLoginScreen(
    ...
    // Navigate on successful login. Faculty app only has one post-login
    // landing: the schedules list. Pop through WELCOME so the post-login
    // root is the schedules screen (so a system-back gesture doesn't land
    // on the pre-login onboarding flow).
    LaunchedEffect(uiState.loginSuccess) {
        if (uiState.loginSuccess) {
            if (uiState.userRole == "faculty") {
                navController.navigate(FacultyRoutes.SCHEDULES) {
                    popUpTo(FacultyRoutes.WELCOME) { inclusive = true }
                }
            } else {
                toastState.showToast(
                    "This app is for faculty only. Install the IAMS Student app to sign in as a student.",
                    ToastType.ERROR,
                )
                viewModel.clearError()
            }
